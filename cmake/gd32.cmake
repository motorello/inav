include(gd32-bootloader)
include(gd32f3)

# As a general rule of thumb, for the time-being, let's assume that GD32 chips are very similar to STM32, so let's
# stick with STM32 CMSIS instead of rewriting all the drivers to be compatible with GD32 BSP
...
set(GD32_STARTUP_DIR "${MAIN_SRC_DIR}/startup")

main_sources(GD32_VCP_SRC
    drivers/serial_usb_vcp.c
    drivers/usb_io.c
)

main_sources(GD32_SDCARD_SRC
...
# XXX: This code is not GD32 specific
main_sources(GD32_ASYNCFATFS_SRC
    io/asyncfatfs/asyncfatfs.c
    io/asyncfatfs/fat_standard.c
)

main_sources(GD32_MSC_SRC
    msc/usbd_storage.c
)

main_sources(GD32_MSC_FLASH_SRC
    msc/usbd_storage_emfat.c
    msc/emfat.c
    msc/emfat_file.c
)

main_sources(GD32_MSC_SDCARD_SRC
...
set(GD32_DEFINITIONS
)
set(GD32_DEFAULT_HSE_MHZ 8)
set(GD32_LINKER_DIR "${MAIN_SRC_DIR}/target/link")
set(GD32_COMPILE_OPTIONS
    -ffunction-sections
    -fdata-sections
    -fno-common
)

set(GD32_LINK_LIBRARIES
    -lm
    -lc
)

if(SEMIHOSTING)
    list(APPEND GD32_LINK_LIBRARIES --specs=rdimon.specs -lrdimon)
    list(APPEND GD32_DEFINITIONS SEMIHOSTING)
else()
    list(APPEND GD32_LINK_LIBRARIES -lnosys)
endif()

set(GD32_LINK_OPTIONS
    -nostartfiles
    --specs=nano.specs
    -static
    -Wl,-gc-sections
    -Wl,-L${GD32_LINKER_DIR}
    -Wl,--cref
    -Wl,--no-wchar-size-warning
    -Wl,--print-memory-usage
)

macro(get_gd32_target_features output_var dir target_name)
    ...
    if(NOT _result EQUAL 0)
        message(FATAL_ERROR "error extracting features for gd32 target ${ARGV2}: ${_errors}")
    endif()
...
function(get_gd32_flash_size out size)
...
function(set_linker_script target script)
    set(script_path ${GD32_LINKER_DIR}/${args_LINKER_SCRIPT}.ld)
...
function(add_gd32_executable)
    ...
    target_include_directories(${elf_target} PRIVATE ${CMAKE_CURRENT_SOURCE_DIR} ${args_INCLUDE_DIRECTORIES} ${GD32_INCLUDE_DIRS})
    target_compile_definitions(${elf_target} PRIVATE ${args_COMPILE_DEFINITIONS})
    target_compile_options(${elf_target} PRIVATE ${GD32_COMPILE_OPTIONS} ${args_COMPILE_OPTIONS})
    ...
    target_link_libraries(${elf_target} PRIVATE ${GD32_LINK_LIBRARIES})
    target_link_options(${elf_target} PRIVATE ${GD32_LINK_OPTIONS} ${args_LINK_OPTIONS})
...
function(target_gd32)
    ...
    if (args_HSE_MHZ)
        ...
        set(hse_mhz ${GD32_DEFAULT_HSE_MHZ})
    endif()

    set(target_sources ${GD32_STARTUP_DIR}/${args_STARTUP})
    ...
    set(target_definitions ${GD32_DEFINITIONS} ${COMMON_COMPILE_DEFINITIONS})

    get_gd32_target_features(features "${CMAKE_CURRENT_SOURCE_DIR}" ${name})
    set_property(TARGET ${elf_target} PROPERTY FEATURES ${features})

    if(VCP IN_LIST features)
        list(APPEND target_sources ${GD32_VCP_SRC} ${args_VCP_SOURCES})
        list(APPEND target_include_directories ${args_VCP_INCLUDE_DIRECTORIES})
    endif()
    if(SDCARD IN_LIST features)
        list(APPEND target_sources ${GD32_SDCARD_SRC} ${GD32_ASYNCFATFS_SRC})
    endif()

    set(msc_sources)
    if(NOT args_DISABLE_MSC AND MSC IN_LIST features)
        list(APPEND target_include_directories ${args_MSC_INCLUDE_DIRECTORIES})
        list(APPEND msc_sources ${GD32_MSC_SRC} ${args_MSC_SOURCES})
        list(APPEND target_definitions USE_USB_MSC)
        if(FLASHFS IN_LIST features)
            list(APPEND msc_sources ${GD32_MSC_FLASH_SRC})
        endif()
        if (SDCARD IN_LIST features)
            list(APPEND msc_sources ${GD32_MSC_SDCARD_SRC})
        endif()
    endif()
